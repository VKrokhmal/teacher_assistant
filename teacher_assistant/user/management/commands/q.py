...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--teacher", type=int, help="Number of Teacher objects to create"
        )
        parser.add_argument(
            "--student", type=int, help="Number of Student objects to create"
        )
    ...
    def handle(self, *args, **options):
        ...
        if num_teachers is not None:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created {num_teachers} Teacher objects."
                )
            )
        print(teachers)
        for teacher in teachers:
            if num_students is not None:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully created {num_students} Student objects."
                    )
                )
